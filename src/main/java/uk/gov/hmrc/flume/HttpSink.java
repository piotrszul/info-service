 ...
 *
 * Behaviour of this sink is that it will take events from the channel, and send
 * those events to a remote service using an HTTP POST request. The event
 * content is sent as the POST body.
 *
 * Configurable options are : endpoint - the fully qualified URL endpoint to
 * POST to (required, no default) connectTimeout - the socket connection timeout
 * (default 5000ms) requestTimeout - the maximum request processing time
 * (default 5000ms) contentTypeHeader - the HTTP Content-Type header (default
 * text/plain) acceptHeader - the HTTP Accept header value (default text/plain)
 * defaultBackoff - whether the default result for event processing is to
 * BACKOFF (default false) defaultRollback - whether by default the event
 * processing transaction is rolled back (default false) defaultIncrementMetrics
 * - whether by default the event contributes to the sink metrics (default true)
 * backoff.[code] - whether to return a BACKOFF (if true) or READY (if false)
 * result when the given HTTP response code is returned from the remote host
 * rollback.[code] - whether the transaction is rolled back (if true) or
 * committed (if false) when the given HTTP response code is returned from the
 * remote host incrementMetrics.[code] - whether to increment the sink metrics
 * when the given HTTP response code is returned from the remote host
 *
 * Note that the values for [code] above can be either a single HTTP status
 * code, i.e. 503 or 200, or a group of one hundred status codes, i.e. 2XX or
 * 5XX. If a single code and a group are specified, then the most specific
 * result is used. So for example both 503 and 5XX may be defined, and the 5XX
 * case will apply for all codes in that range except 503.
 *
...
public class HttpSink extends AbstractSink implements Configurable {
	...
	public void configure(Context context) {
		...
		// TODO: Add validation. Only POST and PUT should be supported
		method = context.getString("method", DEFAULT_METHOD);
		LOG.info("Using method value : " + method);

		username = context.getString("username", null);
		LOG.info("Using method value : " + username);

		StrSubstitutor resolver = new StrSubstitutor(System.getenv());
		if (username != null) {
			password = resolver.replace(context.getString("password"));
			LOG.info("Using password value for user: {}", username);
		}
			...
			if (eventBody != null && eventBody.length > 0) {
				...
				try {
					...
					if (httpStatusCode >= 100) {
						...
						boolean shouldRollback = findOverrideValue(httpStatusString, rollbackOverrides,
								defaultRollback);
						if (shouldRollback) {
							txn.rollback();
						} else {
						...
						boolean shouldIncrementMetrics = findOverrideValue(httpStatusString, incrementMetricsOverrides,
	...
	private void parseConfigOverrides(String propertyName, Context context, Map<String, Boolean> override) {
		...
		if (config != null) {
			for (Map.Entry<String, String> value : config.entrySet()) {
				LOG.info(String.format("Read %s value for status code %s as %s", propertyName, value.getKey(),
						value.getValue()));
